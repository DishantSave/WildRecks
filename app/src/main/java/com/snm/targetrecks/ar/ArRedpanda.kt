...
class ArRedpanda : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        videoNode = VideoNode(sceneView.engine, scaleToUnits = 0.7f, centerOrigin = Position(y=-4f), glbFileLocation = "", player = mediaPlayer, onLoaded = {_,_ ->
            mediaPlayer.start()
        })

        modelNode = ArModelNode(sceneView.engine,PlacementMode.INSTANT).apply {
            loadModelGlbAsync(
                glbFileLocation = "redpanda1.glb",
                scaleToUnits = 20f,
